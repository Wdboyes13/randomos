...
static uintptr_t lapic_phys_addr = 0xFEE00000;
static volatile u32* lapic_virt_addr = NULL;
static u32 bsp_lapic_id = 0;
...
static void parse_madt(void* madt) {
    ...
    while (ptr < end) {
        ...
        switch (entry->type) {
            case ENT_PROCLOCAL_APIC: {
                madt_plapic_t* plapic = (madt_plapic_t*)ptr;
                if (plapic->flags & 1) {
                    if (num_ioapics == 0) {
                        bsp_lapic_id = plapic->apicid;
                    }
                }
                break;
            }
