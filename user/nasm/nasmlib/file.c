...
FILE *nasm_open_read(const char *filename, enum file_flags flags)
{
    ...
    if (!f && (flags & NF_FATAL))
        nasm_fatalf(ERR_NOFILE, "unable to open input file: `%s'",
                    filename);
...
FILE *nasm_open_write(const char *filename, enum file_flags flags)
{
    ...
    if (!f && (flags & NF_FATAL))
        nasm_fatalf(ERR_NOFILE, "unable to open output file: `%s'",
                    filename);
 ...
 */
bool nasm_file_time(u64 *t, const char *pathname)
{
#ifdef os_stat
    os_filename osfname;
    os_struct_stat st;
